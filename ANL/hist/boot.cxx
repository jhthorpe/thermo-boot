...
int main(int argc, char* argv[])
{
    ...
    to_file("original.hist", molecule_set);
    to_file("trial.hist", bs.trial_buf());

    data_buffer_t hist1(N_total);
    data_buffer_t hist2(N_total);
    data_buffer_t hist3(N_total);
    idx_buffer_t iwork(N_total);

    bootstrap::load_random_samples_with_replacement(N_total, hist1, iwork, N_total, molecule_set, rng);
    bootstrap::load_random_samples_with_replacement(N_total, hist2, iwork, N_total, molecule_set, rng);
    bootstrap::load_random_samples_with_replacement(N_total, hist3, iwork, N_total, molecule_set, rng);

    to_file("hist1.hist", hist1);
    to_file("hist2.hist", hist2);
    to_file("hist3.hist", hist3);
