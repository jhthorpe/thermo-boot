...
###########################################################################
# STEP 3 : Fit number of atoms + expdist
# Doesn't help much actually
###########################################################################

#constant plus num e plus num e pair
func <- function(df, a, a_C=0, a_N=0, a_O=0, b_C=0, b_N=0, b_O=0)
{
  return(
    a 
    + (a_C)*df$X.C*4*3/2 + (a_N)*df$X.N*5*4/2 + (a_O)*df$X.O*6*5/2
    + df$CCSDt_CBS1_extdist_q*((b_C)*df$X.C*4*3/2 + (b_N)*df$X.N*5*4/2 + (b_O)*df$X.O*6*5/2)
  )
