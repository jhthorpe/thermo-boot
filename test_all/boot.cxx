 ...
 */
auto unique_subsamples(const idx_t          n_strata, 
                             ...
                             rng_t&         rng) 
{
    ...
    for (int i = 0; i < n_strata; i++)
        indices[i] = std::move(get_unique_subsamples_idx(counts[i], samples[i], rng));
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < N_strata; i++)
        samples[i] = std::move(get_samples_from_file("data_" + std::to_string(i) + ".csv")); 
    ...
    for (idx_t resample = 0; resample < N_RESAMPLE; resample++)
    {
        ...
        bs.sample(N_TRIAL, molecule_set.begin(), molecule_set.end(), trad_conf, rng);
